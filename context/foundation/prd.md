...
**Primary persona:** A Commander/EDH player upgrading a precon or budget deck. They found a target deck list online (on a deck-building site, a content creator's list, or a friend's deck) and want to plan the upgrade path — what to buy, what to cut, grouped by card function. They are comfortable with text-based deck lists (the standard MTG deck-list format) and want to move quickly from "I found a better list" to "here's my shopping list."

## Success Criteria

### Primary
- User pastes two Commander deck lists (base and target) and automatically receives a grouped upgrade plan showing cards to remove and cards to add — organized by card type (lands, creatures, instants, sorceries, artifacts, enchantments) — with card images and approximate prices, plus the total upgrade cost.

### Secondary
- Past comparisons are saved on-device so the user can revisit a previous upgrade plan without re-pasting the lists.

### Guardrails
- Card data accuracy: card names, types, images, and prices must match the authoritative card data source. Misidentified cards make the tool untrustworthy.
...
- Card types, images, and prices are resolved from an authoritative card data source
...
- FR-005: User can see card images for each card in the upgrade plan. Priority: must-have
  > Socrates: Counter-argument considered: "images slow down the page significantly when loading 100+ cards." Resolution: kept; images are core to the MTG tool experience. Performance mitigation is an implementation concern, not a product concern.
...
### Persistence
- FR-009: User can save and revisit past comparisons from on-device storage. Priority: nice-to-have
  > Socrates: Counter-argument considered: "on-device storage is fragile — clearing browser data loses everything." Resolution: kept as nice-to-have; the fragility is accepted for MVP simplicity. No false promise of durability.

## Non-Functional Requirements

- The product remains usable on the latest versions of Chrome, Firefox, Edge, and Safari. No install, extension, or plugin required.
- No user data (deck lists, comparisons, preferences) leaves the user's device except for card-data API lookups by card name. The app does not operate a backend that receives or stores user content.
...
Single user; no auth; data lives on-device only. No roles, no login flow. Past comparisons are persisted locally for revisiting but never leave the device.
...
No open questions at this time — all discovery phases completed with quality check accepted.
